 ...
 * package, which is the whole point: both the PRICING path (`voice-pricing.ts`)
 * and the VALIDATION path (`rate-card.ts`) need this table, and `rate-card`
 * cannot import `voice-pricing` because `voice-pricing` imports `rate-card`.
 *
 ...
 * bundled map silently fell through to the public list price. The user declared
 * a rate, the guard used someone else's number, and nothing said a word.
 *
 * One table, imported by both sides, cannot drift from itself.
 *
 * Port of `src/floe_guard/leg_units.py`.
 */
 ...
 * `voiceLegCost`, and sourced entries in the cost map. A mode with no unit fails
 * closed on every call, which is worse than not having the mode at all.
 */
 ...
 */
export const unitForMode: Record<LegMode, string> = {
  stt: "usd_per_second",
...
export const LEG_MODES: readonly string[] = Object.keys(unitForMode);
